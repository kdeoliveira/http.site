...
const CaretJS = {
    ...
    update(e) {
        ...
        document.querySelector('#' + input.id + ' + span').innerHTML = '&nbsp;'.repeat(pos + diff) + '<span class="custom-caret"></span>';
    },
    ...
    appendSpan(caret) {
        ...
        tmp.innerHTML = '<span class="custom-caret"/>'
        
        // tmp.style.left = parseFloat(s['margin-right']) + parseFloat(s['border-right-width']) + parseFloat(s['padding-right']) + parseFloat(s['width']) + 0.5 + 'px';
    ...
    initialize() {
        window.onload = () => {
            for(var id in CaretJS.carets){
                ...
                document.querySelector('#' + CaretJS.carets[id].input.id + ' + span').innerHTML = '<span class="custom-caret"></span>';
            }
        };
    }
}
